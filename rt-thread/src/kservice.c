...
#define FINSH_USING_CTRL_PIN
#define  FINSH_CTRL_PIN GET_PIN(A, 8)
#define FINSH_485_RCV()  rt_pin_write(FINSH_CTRL_PIN, PIN_LOW)
#define FINSH_485_SEND() rt_pin_write(FINSH_CTRL_PIN, PIN_HIGH)
/**
...
{
    rt_kprintf("\n \\ | /\n");
    rt_kprintf("- OceansKing -    \n");
    rt_kprintf(" / | \\   CCR_PWM  %d.%d.%d build %s %s\n",
               CCR_VERSION, CCR_SUBVERSION, CCR_REVISION, __DATE__, __TIME__);
}
...
rt_device_t rt_console_set_device(const char *name)
{
    ...
    if (new_device != RT_NULL)
    {
        ...
        /** rs485 ctrl*/
        #ifdef FINSH_USING_CTRL_PIN
        rt_pin_mode(FINSH_CTRL_PIN, PIN_MODE_OUTPUT);
        FINSH_485_RCV();
        #endif
    }
